  ...
  <el-container>
    <el-card style="margin-top:20px" >
      <el-form :inline="true" :model="formInline" class="demo-form-inline" size="mini" label-width="80px">
        ...
        <el-form-item>
          <el-button type="primary" @click="onSubmit"><i class="el-icon-search"/>查询</el-button>
        </el-form-item>
        <el-form-item>
    ...
    <el-card class="elMain" style="margin-top:10px">
      <el-button type="primary" size="mini" :disabled="disabled" style="float:left" @click="addList()">
        <i class="el-icon-circle-plus-outline"/>&nbsp;新增</el-button><br><br>
      <el-table v-loading="loading" :data="tableData" stripe border size="mini">
        <el-table-column align="center" label="编号" prop="uId"/>
        <el-table-column align="center" label="用户名" prop="userName"/>
        <el-table-column align="center" label="姓名" prop="uName"/>
    ...
    <add-user :option="addIn" @refreshTable="getData"/>
  </el-container>
</template>
      ...
      commonAPI('queryUserList', this.formInline)
        .then(res => {
          ...
          for (let i = 0; i < this.tableData.length; i++) {
            if (this.$store.state.token === 'editor' && this.$store.state.userName !== this.tableData[i].userName) {
